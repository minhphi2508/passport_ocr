...
def build_orientation_candidate(
    ...
    return {
        ...
        "passport_detected": passport is not None,
        "passport_confidence": (
            float(passport["confidence"]) if passport is not None else None
        ),
        "mrz_detected": mrz is not None,
        "mrz_confidence": float(mrz["confidence"]) if mrz is not None else None,
    }


def run_passport_gate(model: YOLO, image: np.ndarray) -> dict[str, Any]:
    original = build_orientation_candidate(
        angle=0,
        image=image,
        detections=detect_objects(model, image),
    )

    if original["passport_detected"] and original["mrz_detected"]:
        return {
            "passport_gate_status": "passport_confirmed",
            "passport_gate_reason": "original_passport_plus_mrz",
    ...
    for angle, rotation_code in ORIENTATION_ROTATIONS.items():
        rotated = cv2.rotate(image, rotation_code)
        candidates.append(
            build_orientation_candidate(
                angle=angle,
                image=rotated,
                detections=detect_objects(model, rotated),
            )
        )
    ...
    strong = [
        ...
        if candidate["passport_detected"] and candidate["mrz_detected"]
    ]
    ...
    if strong:
        best = max(
            ...
            key=lambda candidate: (
                candidate["passport_confidence"] or 0.0,
                candidate["mrz_confidence"] or 0.0,
            ),
        )
        return {
            "passport_gate_status": "passport_confirmed",
            "passport_gate_reason": "orientation_passport_plus_mrz",
            "orientation_retry_used": True,
            "orientation_angle": int(best["angle"]),
            "selected_candidate": best,
        }

    passport_only = [candidate for candidate in candidates if candidate["passport_detected"]]

    if passport_only:
        best = max(
            passport_only,
            key=lambda candidate: candidate["passport_confidence"] or 0.0,
        )
        return {
            "passport_gate_status": "passport_candidate",
            "passport_gate_reason": "passport_detected_without_mrz",
            "orientation_retry_used": True,
            "orientation_angle": int(best["angle"]),
            "selected_candidate": best,
        }
    ...
    return {
        "passport_gate_status": "no_passport_evidence",
        "passport_gate_reason": "no_passport_detected_any_orientation",
        "orientation_retry_used": True,
