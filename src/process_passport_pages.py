...
def quad_is_valid(
    ...
    # Tứ giác phải phủ gần toàn bộ chiều rộng và chiều cao crop.
    ...
    if mrz_bbox_in_crop is not None:
        mrz_inside_ratio = polygon_bbox_intersection_ratio(
            polygon=ordered,
            bbox=mrz_bbox_in_crop,
            image_shape=(crop_height, crop_width),
        )

        if mrz_inside_ratio < MIN_MRZ_INSIDE_RATIO:
            return (
                False,
                f"mrz_not_fully_inside:{mrz_inside_ratio:.3f}",
            )

        # Cạnh dưới của tứ giác tuyệt đối không được nằm phía trên MRZ.
...
def process_one_image(
    ...
    if detections is None:
        return {
            ...
            "status": "passport_page_not_detected",
            "validation_reason": None,
            "error": None,
        }
    ...
    if mrz is not None:
        mrz_bbox_in_crop = bbox_to_crop_coordinates(
            bbox=mrz["bbox"],
    ...
    transformed, rotated = rotate_to_landscape(
        transformed
    )
    ...
    return {
        ...
        "passport_confidence": round(
            passport["confidence"],
            6,
        ),
        "mrz_confidence": (
            round(mrz["confidence"], 6)
            if mrz is not None
            else None
        ),
...
def main() -> None:
    ...
    print("\n" + "=" * 72)
    print("KẾT QUẢ PERSPECTIVE AN TOÀN")
    print("=" * 72)
